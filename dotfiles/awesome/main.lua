...
local function refresh_monitor_prefixes()
	...
	for s in screen do
		if s.monitor_prefix then
			if n > 1 then
				s.monitor_prefix.markup = string.format(
...
screen.connect_signal("added", function(_)
	refresh_monitor_prefixes()
end)
screen.connect_signal("removed", function(_)
	refresh_monitor_prefixes()
end)
...
awful.spawn.easy_async_with_shell([[sh -c 'nvidia-smi -L >/dev/null 2>&1 && echo YES || echo NO']], function(out)
...
awesome.connect_signal("mode::bar_bg", function(color)
...
awful.screen.connect_for_each_screen(function(s)
	...
	awesome.connect_signal("env::egpu_ready", function(has)
...
globalkeys = gears.table.join(
	...
	end, { description = "lock computer", group = "layout" }),
	--------------------------------------------- FUNCTION KEYS --------------------------------------------
	...
	end, { description = "screenshot: screen", group = "screenshot" }),
	awful.key({ "Shift" }, "Print", function()
		awful.spawn("flameshot gui -p /home/gg/media/img/screenshots/")
	end, { description = "screenshot: gui", group = "screenshot" }),
	awful.key({ modkey }, "Print", function()
		awful.spawn("flameshot full -p /home/gg/media/img/screenshots/")
	end, { description = "screenshot: full", group = "screenshot" }),
	--------------------------------------------- PROGRAMS ------------------------------------------------
	...
	awful.key({ modkey }, "e", function()
		awful.spawn("thunar")
	end, { description = "open file exporer", group = "launcher" }),
	--------------------------------------------- WORKFLOW ------------------------------------------------
	...
	end, { description = "restore from scrap pad", group = "client" })
)
...
clientkeys = gears.table.join(
	...
	awful.key({ modkey }, "q", function()
		...
		local function is_generic_terminal()
			return terminal_classes[class]
				and (
					name == ""
					or name:lower():match("terminal")
					or name:lower():match("^bash$")
					or name:lower():match("^zsh$")
					or name:lower():match("^fish$")
					or name:lower():match("^sh$")
				)
		end
		...
		c:kill()
	end, { description = "send to scrap or close", group = "client" }),
...
for i = 1, 9 do
	globalkeys = gears.table.join(
		...
		end, { description = "move client to screen #" .. i, group = "screen" })
	)
end
...
gears.timer.delayed_call(function()
	...
	local function firefox_running(cb)
		awful.spawn.easy_async_with_shell([[pgrep -x firefox >/dev/null && echo RUNNING || echo NOT]], function(out)
