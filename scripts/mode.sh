...
PERF="$HOME/nixCore/scripts/performance.sh"

usage(){ echo "Usage: mode {server|normal|performance|status}"; exit 1; }
...
case "${1:-}" in
  server|normal|performance)
    ...
    if [[ "$prev" != "performance" && "$new" == "performance" ]]; then
      [[ -x "$PERF" ]] || { echo "Error: $PERF not found/executable"; exit 1; }
      "$PERF" enable
